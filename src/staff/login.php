...
<head>
  ...
  <!-- <body class="contest-page inner-page <? if ($serverPost): ?>overflow-h<? endif; ?>"> -->
    ...
    <section class="user-contacts">
      <form method="post" onsubmit="return validateLogin(this);">
        <fieldset class="user-contacts__fieldset">
          ...
          <div class="user-contacts__block user-contacts__block--password <?= $erorrs[1] ?>">
              <label for="password">Пароль: <span class="error-messange">Длина пароля должна быть от 2 до 30 символов</span></label>
  ...
  <? if (!$validation1 || !$validation2): ?> <section class="modal modal-error">
  <? else: ?><section class="modal modal-error d-none"><? endif; ?>
    <div class="modal_wrapper">
      <div class="modal__main">
      <? if (!$validation1):?>
        <h3 class="modal__title">Ошибка входа</h3>
